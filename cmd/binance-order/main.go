...
import (
	...
	"github.com/dgnydn/binance-order/internal/app/binance-order"
	"github.com/dgnydn/binance-order/internal/app/binance-order/todo/tododriver"
	"github.com/dgnydn/binance-order/internal/common/commonadapter"
	"github.com/dgnydn/binance-order/internal/platform/appkit"
	"github.com/dgnydn/binance-order/internal/platform/database"
	"github.com/dgnydn/binance-order/internal/platform/gosundheit"
	"github.com/dgnydn/binance-order/internal/platform/log"
	"github.com/dgnydn/binance-order/internal/platform/watermill"
)
...
func main() {
	...
	// Set up app server
	{
		...
		// In larger apps, this should be split up into smaller functions
		{
			...
			binance-order.InitializeApp(httpRouter, grpcServer, publisher, config.App.Storage, db, logger, errorHandler)

			h, err := watermill.NewRouter(logger)
			emperror.Panic(err)

			err = binance-order.RegisterEventHandlers(h, subscriber, logger)
			emperror.Panic(err)
